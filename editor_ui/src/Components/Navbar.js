...
const Navbar = ({ language, setLanguage,roomId, accessList, setAccessList, isOwner, socketRef,username }) => {
  const languages = [
    { value: "c", label: "C" },
    { value: "cpp", label: "CPP" },
  ...
  return (
    <div>
      <div className="navbar">
        ...
        />
        {isOwner&&<div className="accessGrp">
        <Select
          className="select"
        ...
        />
        <button className="btn accessBtn" onClick={removeAccess}>Remove Access</button>
        <input className="accessInput" id="accessInput" type="text" onChange={(e)=>setEmail(e.target.value)}></input>
        <button className="btn accessBtn" onClick={giveAccess}>Give Access</button>
        </div>}
      </div>
    </div>
  );
